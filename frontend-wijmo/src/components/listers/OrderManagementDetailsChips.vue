<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>OrderManagementDetails</v-list-item-title>
                    </v-list-item-content>
                </template>
                ...
                <v-list-item v-for="(item, idx) in value" :key="idx">
                    <v-list-item-content>
                        ...
                        <v-list-item-subtitle>
                            Id :  {{item.id }} <br>
                            UniqueNumber :  {{item.uniqueNumber }} <br>
                            ItemCode :  {{item.itemCode }} <br>
                            ItemName :  {{item.itemName }} <br>
                            ItemSpecification :  {{item.itemSpecification }} <br>
                        </v-list-item-subtitle>
                    </v-list-item-content>
        ...
        <OrderManagementDetails v-if="editMode && tick" offline :isNew="true" :inList="true"
                :editMode="editMode" v-model="newValue" @add="append"
        >
            <template slot="actions">
                <v-layout row justify-end>
                    <v-btn style="width:84px; margin:0px 7px 10px 0px;"
                        color="primary"
...
<script>
    ...
    export default {
        name: 'OrderManagementDetailsManager',
        components: {
            OrderManagementDetails
        },
        props: {
